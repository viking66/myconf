"" In ChromeOS we don't have execute permissions from the home directory so
"" move this to /usr/local/vim/vimrc and create symlinks:
"" ln -s /usr/local/vim ~/.vim
"" ln -s ~/.vim/vimrc ~/.vimrc

"" Install vim-plug if it isn't already installed
let shouldInstallPlugins=0
let plug_path=expand('/usr/local/vim/autoload/plug.vim')
if !filereadable(plug_path)
    echo "Installing vim-plug..."
    echo
    silent !mkdir -p /usr/local/vim/autoload
    silent !curl -fLo /usr/local/vim/autoload/plug.vim https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim
...
silent !mkdir -p /usr/local/vim/{backup,tmp}
set backup
set backupdir=/usr/local/vim/backup
set directory=/usr/local/vim/tmp
